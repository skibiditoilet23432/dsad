...
async def summary_and_cleanup(update, context, server_name):
    ...
    if differences:
        ...
        if language == 'vi':
            summary_message = (
                "<code>"
                f"💎 <b>Thống Kê {server_name}</b> 💎\n\n"
                f"➥ 🌟 Tổng Requests: <b>{total_difference:,}</b>\n"
                f"➥ 🚀 Requests/s Cao Nhất: <b>{max_difference:,}</b>\n"
                f"➥ 🎯 Trung Bình Requests/s: <b>{round(average_difference):,}</b>\n\n"
                "</code>"
                f"✨ Dữ liệu từ người dùng: <a href='{user_url}'>🦄 {full_name}</a> 🌐"
            )
        else:
            summary_message = (
                "<code>"
                f"💎 <b>{server_name} Statistics</b> 💎\n\n"
                f"➥ 🌟 Total Requests: <b>{total_difference:,}</b>\n"
                f"➥ 🚀 Peak Requests: <b>{max_difference:,}</b>\n"
                f"➥ 🎯 Average Requests: <b>{round(average_difference):,}</b>\n\n"
                "</code>"
                f"✨ Data from user: <a href='{user_url}'>🦄 {full_name}</a> 🌐"
            )
...
def create_summary_message(language, server_name, total_received, max_received, average_received, total_packet, max_packet, average_packet, update):
    ...
    title = f"{server_name}" if language != 'vi' else f"Máy Chủ {server_name}"

    if language == 'vi':
        content = (
            "<code>\n"
            f"{border_top}"
            + border_mid.format(title=title) +
            f"{border_sep}"
            "║  📡 BĂNG THÔNG                  ║\n"
            f"║    ┣━ 📊 Tổng:      {total_received_formatted:<9}║\n"
            f"║    ┣━ 🚀 Cao nhất:   {max_received_formatted:<9}║\n"
            f"║    ┗━ 🌈 Trung bình: {average_received_formatted:<9}║\n"
            f"{border_sep}"
            "║  📦 GÓI TIN                     ║\n"
            f"║    ┣━ 🔢 Tổng:      {total_packet_formatted:<9}║\n"
            f"║    ┣━ ⚡ Cao nhất/s: {max_packet_formatted:<9}║\n"
            f"║    ┗━ 📥 Trung bình/s: {average_packet_formatted:<6} ║\n"
            f"{border_bot}"
            "</code>\n"
            f"👤 Người dùng: <a href='{user_url}'>🧑‍💻 {full_name}</a> • 🕒 {now}"
        )
    else:
        content = (
            "<code>\n"
            ...
            f"{border_sep}"
            "║  📡 BANDWIDTH                   ║\n"
            f"║    ┣━ 📊 Total:    {total_received_formatted:<9}║\n"
            f"║    ┣━ 🚀 Peak:     {max_received_formatted:<9}║\n"
            f"║    ┗━ 🌈 Average:  {average_received_formatted:<9}║\n"
            f"{border_sep}"
            "║  📦 PACKETS                     ║\n"
            f"║    ┣━ 🔢 Total:    {total_packet_formatted:<9}║\n"
            f"║    ┣━ ⚡ Peak PPS: {max_packet_formatted:<9}║\n"
            f"║    ┗━ 📥 Avg PPS:  {average_packet_formatted:<9}║\n"
            f"{border_bot}"
            "</code>\n"
